...
class GRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        # DASH (ours)
        self.reward_iw = args.reward_iw
        self.iw_clip = args.iw_clip
    ...
    def _prepare_inputs(
        ...
        if mode == "train":
            ...
            # if self.accelerator.is_main_process:
            #     print(f"buffer len is {len(self._buffered_inputs)}")
            #     print(f"current step is {self._step}")
            #     print(f"index id is {self._step % self.args.steps_per_generation}")
    ...
    # def _call_reward_with_guard(self, reward_func, prompts, completions, completion_ids_list, reward_kwargs, 
    #                             per_item_timeout_s=20):
    #     import time
    #     out = []
    #     for idx, (p, c, cid) in enumerate(zip(prompts, completions, completion_ids_list)):
    #         t0 = time.time()
    #         try:
    #             # Small slice of kwargs for this row
    #             row_kwargs = {k: (v[idx] if isinstance(v, list) else v) for k, v in reward_kwargs.items()}
    #             result = reward_func(prompts=[p], completions=[c], completion_ids=[cid], **row_kwargs)
    #             # Normalize return types
    #             if isinstance(result, (list, tuple)):
    #                 if len(result) != 1:
    #                     self.accelerator.print(f"[warn] reward_func returned len={len(result)} for single row {idx}; taking first.")
    #                 result = result[0]
    #             out.append(result)
    #         except Exception as e:
    #             self.accelerator.print(f"[rank {self.accelerator.process_index}] reward_func error on idx={idx}: {e}")
    #             out.append(None)  # will become NaN downstream
    #         dt = time.time() - t0
    #         if dt > per_item_timeout_s:
    #             self.accelerator.print(f"[rank {self.accelerator.process_index}] reward_func timeout on idx={idx} after {dt:.1f}s")
    #             out[-1] = None
    #     return out
    ...
    def _calculate_rewards(self, inputs, prompts, completions, completion_ids_list):
        ...
        # print(f"before this big for loop: {device}")
        for i, (reward_func, reward_processing_class, reward_func_name) in enumerate(
            zip(self.reward_funcs, self.reward_processing_classes, self.reward_func_names)
        ):
            # print(f"inside the big for loop: {device} with index {i}")
            with profiling_context(self, reward_func_name):
                if isinstance(reward_func, nn.Module):  # Module (no PretrainedModel) for compat with compiled models
                    ...
                    # print(f"before reward_func calc, {device}")
                    with torch.no_grad():
                        # print(f"len(completions), {device}")
                        # print(f"len(reward_kwargs['answer']), {device}")
                        # reward_kwargs['answer'] = [a for a in reward_kwargs['answer'] for _ in range(self.num_generations)]
                        # output_reward_func = self._call_reward_with_guard(
                        #     reward_func, prompts, completions, completion_ids_list, reward_kwargs
                        # )
                        # if device == "cuda:3":
                        #     print(completions)
                        #     print(reward_kwargs['answer'])
                    ...
                    # print(f"after reward_func calc, {device}")
    ...
    def _generate_and_score_completions(
        ...
        output = {
            ...
            "rewards": rewards[process_slice],  # for DASH importance weighting; unsliced
    ...
    def _compute_loss(self, model, inputs):
        ...
        if self.reward_iw:
            all_rewards = self.accelerator.gather(inputs["rewards"])
            rewards = inputs["rewards"].unsqueeze(1)
            iw = torch.exp(per_token_logps.detach().sum(dim=1) - old_per_token_logps.sum(dim=1)).unsqueeze(1)
            iw_clipped = torch.clamp(iw, 0.0, self.iw_clip)
            weighted_rewards = rewards * iw_clipped
            all_weighted_rewards = self.accelerator.gather(weighted_rewards)
            mean_weighted_rewards = all_weighted_rewards.view(-1, self.num_generations).mean(dim=1)
            mean_weighted_rewards = mean_weighted_rewards.repeat_interleave(self.num_generations, dim=0)
            all_advantages = all_rewards - mean_weighted_rewards
            process_slice = slice(
        ...
        self._metrics[mode]["policy_ratio"].append(coef_1.detach().mean().item())

        # DASH iw
        if self.reward_iw:
            is_iw_clipped = (iw != iw_clipped)
            iw_clip_ratio = masked_batch_mean(is_iw_clipped.float()) # treated as sequence level
            gathered_iw_clip_ratio = self.accelerator.gather(iw_clip_ratio)
            self._metrics[mode]["iw_clip_ratio"].append(gathered_iw_clip_ratio.nanmean().item())

            self._metrics[mode]["iw_pre_clip"].append(iw.float().mean().item())
            self._metrics[mode]["iw_after_clip"].append(iw_clipped.float().mean().item())
            self._metrics[mode]["weighted_reward"].append(mean_weighted_rewards.mean().item())
            self._metrics[mode]["weighted_advantage"].append(advantages.float().abs().mean().item())
        return loss
